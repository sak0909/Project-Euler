#!/usr/local/bin/python3
#Solution to problem 18: Maximum path sum I
#Dynamic algorithm using memoization
# https://projecteuler.net/problem=18
# https://www.hackerrank.com/contests/projecteuler/challenges/euler018


import math
import fileinput

input = [[75],
         [95, 64],
         [17, 47, 82],
         [18, 35, 87, 10],
         [20, 4, 82, 47, 65],
         [19, 1, 23, 75, 3, 34],
         [88, 2, 77, 73, 7, 63, 67],
         [99, 65, 4, 28, 6, 16, 70, 92],
         [41, 41, 26, 56, 83, 40, 80, 70, 33],
         [41, 48, 72, 33, 47, 32, 37, 16, 94, 29],
         [53, 71, 44, 65, 25, 43, 91, 52, 97, 51, 14],
         [70, 11, 33, 28, 77, 73, 17, 78, 39, 68, 17, 57],
         [91, 71, 52, 38, 17, 14, 91, 43, 58, 50, 27, 29, 48],
         [63, 66, 4, 68, 89, 53, 67, 30, 73, 16, 69, 87, 40, 31],
         [04, 62, 98, 27, 23, 9, 70, 98, 73, 93, 38, 53, 60, 4, 23]]

ans = [[0 for col in range(len(input))] for row in range(len(input))]
def MaxPath(input, i, j):
  if i >= len(input):
    return 0
  if j >= len(input[i]):
    return 0
  if i == len(input)-1:
    ans[i][j] = input[i][j]
    return input[i][j]
  if ans[i][j] == 0:
    val = max( MaxPath(input, i+1, j), MaxPath(input, i+1, j+1) )
    ans[i][j] = input[i][j] + val
    print("Returning " + str(input[i][j]) + "+" + str(val))
  return ans[i][j]

t = fileinput.input()
for i in t:
    LN = [];
    N = fileinput.input()
    for j in N:
        numbers = [int(n) for n in input().split()]
        LN.append(numbers)
    print(MaxPath(LN, 0, 0))


    
  
